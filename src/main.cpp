...
uint256 hashGenesisBlock("0x5ecf088a0c7eb7031b76a45c34d68e40b8a365a2e0fded1dc6e8f978dad98f1d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Atcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Atcoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 86400); // Atcoin: 1.0512mil blocks in ~1 year

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60;	// Atcoin: 1 hour
static const int64 nTargetSpacing = 60;		// Atcoin: 30 sec
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int64 nTargetTimespanRe = 60 * 60; // 60 Minutes
static const int64 nTargetSpacingRe = 30; // Atcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
   ...
   if(fNewDifficultyProtocol) {
    ...
    // Atcoin: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal atcoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Atcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Altman played monster";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1393855209;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1919371;
        ...
        assert(block.hashMerkleRoot == uint256("0xa60f8eac90f88d0965f5b36db4912861f7df315ac3a169ae3d115cd02526a9e7"));
...
{
	return false;
	//commenting out this code until we properly implement ATC alerts.
	/*
    if (!CheckSignature())
        return false;
    if (!IsInEffect())
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Atcoin: increase each by adding 2 to bitcoin's value.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Atcoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
